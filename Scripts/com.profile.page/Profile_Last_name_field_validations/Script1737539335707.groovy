...
TestObject firstNameField = findTestObject('Page_Create Account  NJ Transit/label_Last Name')

String FN_Text = WebUI.getText(firstNameField)

println('labelText :' + FN_Text)

WebUI.verifyMatch(FN_Text, 'Last Name *', false)

// Verify the placeholder value
TestObject FN_placeholder = findTestObject('UserRegistration_object_Repository/Page_Create Account  NJ Transit/input_Last Name')

WebUI.sendKeys(FN_placeholder, Keys.chord(Keys.CONTROL, 'a') + Keys.BACK_SPACE)

WebUI.verifyElementAttributeValue(FN_placeholder, 'placeholder', 'Last Name', 10)
