...
g_autoreloadData = CustomKeywords.'com.web.db.Validate_g_autoreload.get_g_autoreload_data_from_DB'(Web_db_connection, GlobalVariable.Email)

println('Auto-reload Data from DB: ' + g_autoreloadData)

// Get USER_ID from DB
def dbUserId = g_autoreloadData['USER_ID']
...
//status
def dbStatus = g_autoreloadData['STATUS']

WebUI.verifyMatch('1', dbStatus, false)

//card no
def dbCardNo = g_autoreloadData['CARD_NO']
...
def dbReloadAmount = (g_autoreloadData['RELOAD_AMT']).toString()
...
def dbThresholdAmount = (g_autoreloadData['THRESHOLD_AMT']).toString()
