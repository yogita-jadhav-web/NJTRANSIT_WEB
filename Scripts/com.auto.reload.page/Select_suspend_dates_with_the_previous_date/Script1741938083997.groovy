...
yesterday_date = CustomKeywords.'kw_web.ApplicationFunction.get_yesterday_date_dd'()

TestObject dateInput = findTestObject('Page_Fare-Pay Dashboard  NJ Transit/button_date_input')

String dynamicXPath = ('//button[text()=\'' + yesterday_date) + '\']'

TestObject dateCell = new TestObject().addProperty('xpath', ConditionType.EQUALS, dynamicXPath)

//WebUI.click(dateCell)

WebUI.comment('User should not be able to select the past date :' + yesterday_date)
println('User should not be able to select the past date :' + yesterday_date)
