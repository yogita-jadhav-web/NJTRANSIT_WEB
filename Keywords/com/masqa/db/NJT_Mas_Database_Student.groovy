...
public class NJT_Mas_Database_Student {
	...
	public Connection get_MySQL_Mas_DBConnection(String NJT_Mas_DB_Url,String NJT_Mas_DB_UserName,String NJT_Mas_DB_Password) {
		...
		if(con==null) {
			...
			try {
				...
				if(con!=null) {
					println(" MYSQL NJT MAS Database Connection Successfull")
				}
				else {
					println(" MYSQL NJT MAS Database Connection Failed !........")
				}
	...
	public String get_Valid_OTP_from_DB(Connection db_conn_obj, String email_id) {
		...
		try {
			...
			stmt = con.createStatement();
			String sql_query = "select MESSAGE_BODY from masqa.p_email_message where RECIPIENTS = '"+email_id+"' order by SEND_DATE desc";
	...
	@Keyword
	public String get_verificationLink_from_DB(Connection db_conn_obj, String email_id) {
		...
		while(rs.next()) {
			...
			if (matcher.find()) {
				...
				println('No verification link found in the email body.')
			}
		}
	...
	public String verify_text_present_in_subject(Connection db_conn_obj, String email_id) {
		...
		while(rs.next()) {
			...
			subject = rs.getString('MESSAGE_BODY');
